...
module.exports = {
  init(config) {
    ...
    {
      const makeUrl = async () => {
        let localUrl = await localUpload.init().upload(file);
        console.dir('localUrl');
        console.dir(localUrl);
        let url = `https://${configs.domain}${configs.isV7 ? '/v7' : ''}/www.sample.li/birds.jpg`; // Fix this later configs.ciDomain, configs.isV7, file.url.replace(/^https?:\/\//, '') 
        file.url = url;
      }
